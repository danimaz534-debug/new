drop function if exists public.create_order_from_cart(text, jsonb) CASCADE;
drop function if exists public.ensure_chat_thread() CASCADE;
drop function if exists public.generate_wholesale_code() CASCADE;
drop function if exists public.redeem_wholesale_code(text) CASCADE;
drop function if exists public.ensure_profile(text, text, text) CASCADE;
...
for all using (public.current_role() in ('admin', 'marketing'))
with check (public.current_role() in ('admin', 'marketing'));
...
create policy "users manage history" on public.watch_history
for all using (user_id = auth.uid())
with check (user_id = auth.uid());

create policy "users read own orders" on public.orders
for select using (user_id = auth.uid() or public.current_role() in ('admin', 'sales'));
...
create policy "order items readable by participants" on public.order_items
for select using (
  order_id in (
    select o.id
...
create policy "order items insert by participants" on public.order_items
...
for insert with check (user_id = auth.uid() or public.current_role() in ('admin', 'sales'));
...
for delete using (user_id = auth.uid() or public.current_role() in ('admin', 'sales'));
...
create policy "reviews are public readable" on public.reviews
for select using (true);

create policy "chat thread access" on public.chat_threads
for select using (user_id = auth.uid() or assigned_sales_id = auth.uid() or public.current_role() in ('admin', 'sales'));

create policy "chat thread create" on public.chat_threads
for insert with check (user_id = auth.uid() or public.current_role() in ('admin', 'sales'));

create policy "chat thread update" on public.chat_threads
for update using (user_id = auth.uid() or assigned_sales_id = auth.uid() or public.current_role() in ('admin', 'sales'))
with check (user_id = auth.uid() or assigned_sales_id = auth.uid() or public.current_role() in ('admin', 'sales'));
...
create policy "chat message create" on public.chat_messages
for insert with check (
  thread_id in (
    select t.id
    from public.chat_threads t
...
create policy "sales read wholesale codes" on public.wholesale_codes
for select using (public.current_role() in ('admin', 'sales'));

create policy "sales create wholesale codes" on public.wholesale_codes
for insert with check (public.current_role() in ('admin', 'sales'));

create policy "wholesale code redeemer update" on public.wholesale_codes
for update using (public.current_role() in ('admin', 'sales') or redeemed_by = auth.uid())
with check (public.current_role() in ('admin', 'sales') or redeemed_by = auth.uid());
